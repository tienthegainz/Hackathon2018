            ...
            for i in range(1,length):
                ...
                if(x != None) and (y != None):
                    if (position[0] < x + VCH_DEVIATION) and (position[0] > x - VCH_DEVIATION) and (position[1] < y + VCH_DEVIATION) and (position[1] > y - VCH_DEVIATION):
                        is_vehicle_detected = 0
                        # print("detected: ", is_vehicle_detected,"OBJECT: ", obj_name, " len_list_vch: ", len(vch[obj_name]))
                        vch[obj_name][i]['x'] = int(position[0])
                        vch[obj_name][i]['y'] = int(position[1])
                        break
                    else:
                        is_vehicle_detected = 1
            vch[obj_name][length]={'x': int(position[0]), 'y': int(position[1])}
    # print("detected: ", is_vehicle_detected)
    return (is_vehicle_detected, vch)
